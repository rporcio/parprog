...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def upsweep(f: Int, t: Int, maxAngle: Float): Float = {
      if (f < t) upsweep(f + 1, t, max(maxAngle, input(f) / f))
      else maxAngle
    }

    upsweep(from, until, 0)
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val med = from + (end - from) / 2
      val (left, right) =
        parallel(upsweep(input, from, med, threshold), upsweep(input, med, end, threshold))
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def rec(f: Int, t: Int, angle: Float): Unit =
      if (f < t) {
        output(f) = max(input(f) / f, angle)
        rec(f + 1, t, output(f))
      }
